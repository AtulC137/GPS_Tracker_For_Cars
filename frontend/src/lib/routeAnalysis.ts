...
export function segmentTrips(
  ...
  const trips: HistoryPoint[][] = [[points[0]]];
...
function distanceOfPoints(points: HistoryPoint[]): number {
  let km = 0;
  for (let i = 1; i < points.length; i++) {
    km += haversineKm(
      points[i - 1].latitude,
...
function countStops(points: HistoryPoint[]): number {
...
function movingIdleSec(points: HistoryPoint[]): { movingSec: number; idleSec: number } {
  let movingSec = 0;
  let idleSec = 0;
...
export function computeRouteStats(points: HistoryPoint[]): RouteStats {
  if (points.length === 0) {
    return {
      pointCount: 0,
      firstTimestamp: null,
...
export function filterPointsByTrip(
  points: HistoryPoint[],
  trip: TripSummary,
): HistoryPoint[] {
